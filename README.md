...
> In 'Delete Marker Removal' mode, it will simply remove delete markers placed after the specified time, from objects where a delete marker is the current version. See [mode-comparison](#mode-comparison) for details.
...
5. **Start S3 Batch Operations jobs**: The tool creates one or more S3 Batch Operations jobs to carry out S3 operations, but by default it will *not* start them. If you are working with a test dataset and do not need to validate the operations or adjust permissions (e.g. [KMS](#kms-permissions)), change this to `Yes`.
6. **Copy to storage class**: The Amazon S3 storage class to copy object versions into. [If in doubt, use the Intelligent-Tiering storage class](https://catalog.workshops.aws/awscff/en-US/playbooks/storage/s3/2-choosing-storage-class). See [the S3 documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/sc-howtoset.html) for more information.
7. **Copy using KMS key (optional)**: If object versions should be created using a KMS encryption key, specify it here. Leave blank for bucket default. *Permissions to KMS keys are not updated by this tool - see the* [**KMS permissions**](#kms-permissions) *section.*
...
5. Deploy the [CloudFormation template](s3-rollback.yaml).
   ...
   - Set **Specify CSV inventory** to the URI of the real-time CSV inventory object you uploaded, or leave this blank if you enabled S3 Metadata.
   - **Create Stack**.
